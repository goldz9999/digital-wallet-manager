            ...
            <TableBody>
              {mappings.length === 0 ? (
                ...
                <TableRow key={m.id} className="border-border/50">
                  ...
                  <TableCell className="text-right font-mono text-sm">{m.cost > 0 ? `$${Number(m.cost).toFixed(2)}` : <span className="text-muted-foreground text-xs">—</span>}</TableCell>
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-3">
                                {selectedSupplierProduct.fixedPrices?.map((price) => (
                                  <button key={price} onClick={() => setWizard(w => ({ ...w, selectedFixedPrice: price }))} className={cn("px-6 py-3 rounded-xl border text-sm font-mono font-semibold transition-all", wizard.selectedFixedPrice === price ? "border-primary bg-primary/15 text-primary" : "border-border bg-secondary/30 hover:bg-secondary/60 text-muted-foreground")}>${Number(price).toFixed(2)}</button>
                                ))}
                              </div>
                            </div>
                          ) : (
